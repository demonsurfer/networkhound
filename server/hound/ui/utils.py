...
from hound.ui import constants
from config import hound_SERVER_HOME
import os
from werkzeug.contrib.cache import SimpleCache
import socket
import struct
from hound.api.models import Sensor
import geoip2.database

flag_cache = SimpleCache(threshold=1000, default_timeout=300)
sensor_cache = SimpleCache(threshold=1000, default_timeout=300)

geoip2_reader = geoip2.database.Reader(hound_SERVER_HOME+'/../../GeoLite2-City.mmdb')
...
def _get_flag_ip_localdb(ipaddr):
    ...
    try:
        ...
        if os.path.exists(hound_SERVER_HOME +"/hound"+flag):
            return flag
        else:
            return constants.DEFAULT_FLAG_URL
