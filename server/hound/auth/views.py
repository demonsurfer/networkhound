...
from hound import db, mail
from hound import user_datastore
from hound.common.utils import error_response
from hound.auth.models import User, PasswdReset, ApiKey
from hound.auth import errors
from hound.auth import (
    get_datastore, login_required, roles_accepted, current_user)
from hound.api import errors as apierrors
import uuid
...
def reset_passwd_request():
    ...
    msg = Message(
            html=reset.email_body, subject='hound Password reset',
            recipients=[user.email], sender=hound.config['DEFAULT_MAIL_SENDER'])
    try:
        mail.send(msg)
    except:
