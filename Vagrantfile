...
Vagrant.configure("2") do |config|
	...
	config.vm.define "ubuntu1604" do |ubuntu1604|
		...
		ubuntu1604.vm.provision "dependencies", type: "shell", run: "never" do |u16ds|
			$u16dscript = <<-SCRIPT
			sudo git clone https://github.com/demonsurfer/hound.git /opt/hound/
		...
		ubuntu1604.vm.provision "shell", run: "never" do |u16s|
			$u16script = <<-SCRIPT
			sudo git clone https://github.com/demonsurfer/hound.git /opt/hound/
			sudo bash /opt/hound/install.sh
			SCRIPT
	...
	config.vm.define "ubuntu1804" do |ubuntu1804|
		...
		ubuntu1804.vm.provision :shell, inline: 'sudo git clone https://github.com/demonsurfer/hound.git /opt/hound/'
		ubuntu1804.vm.provision :shell, inline: 'sudo bash /opt/hound/install.sh'
	end
	...
	config.vm.define "centos6" do |centos6|
		...
		centos6.vm.provision "dependencies", type: "shell", run: "never" do |c6ds|
			...
			sudo bash /opt/hound/scripts/install_sqlite.sh
			sudo bash /opt/hound/scripts/install_supervisord.sh
			sudo bash /opt/hound/scripts/install_hpfeeds.sh
			sudo bash /opt/hound/scripts/install_mnemosyne.sh
			sudo bash /opt/hound/scripts/install_honeymap.sh
			SCRIPT
