...
cat > /etc/logstash/conf.d/hound.conf <<EOF
input {
  file {
    path => "/var/log/hound/hound-json.log"
  }
}
...
output {
  elasticsearch {
    ...
    index => "hound-%{+YYYYMMddHH00}"
    template_name => "hound_event"
    template => "/etc/logstash/conf.d/hound-template.json"
...
cat > /etc/logstash/conf.d/hound-template.json <<EOF
{
  "index_patterns": "hound-*",
  "settings": {
    "number_of_shards": 5,
